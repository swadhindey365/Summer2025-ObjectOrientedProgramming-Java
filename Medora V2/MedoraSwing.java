...
private void showUpdateTotalBedsDialog() {
    ...
    // Current totals
    int currentTotalBeds = currentHospital.getTotalEconomyBeds() + currentHospital.getTotalVipBeds() + currentHospital.getTotalIcuBeds();
    
    // Display current bed distribution
    JLabel currentLabel = new JLabel("Current Total Beds: " + currentTotalBeds);
    ...
    // Economy beds (can increase/decrease)
    JLabel economyLabel = new JLabel("Economy Beds (can increase/decrease):");
    JTextField economyField = new JTextField(String.valueOf(currentHospital.getTotalEconomyBeds()), 10);
    gbc.gridx = 0; gbc.gridy = 1; formPanel.add(economyLabel, gbc);
    gbc.gridx = 1; formPanel.add(economyField, gbc);

    // VIP beds (can increase/decrease)
    JLabel vipLabel = new JLabel("VIP Beds (can increase/decrease):");
    JTextField vipField = new JTextField(String.valueOf(currentHospital.getTotalVipBeds()), 10);
    gbc.gridx = 0; gbc.gridy = 2; formPanel.add(vipLabel, gbc);
    gbc.gridx = 1; formPanel.add(vipField, gbc);

    // ICU beds (can increase/decrease)
    JLabel icuLabel = new JLabel("ICU Beds (can increase/decrease):");
    JTextField icuField = new JTextField(String.valueOf(currentHospital.getTotalIcuBeds()), 10);
    gbc.gridx = 0; gbc.gridy = 3; formPanel.add(icuLabel, gbc);
    gbc.gridx = 1; formPanel.add(icuField, gbc);
    ...
    if (result == JOptionPane.OK_OPTION) {
        try {
            int economyBeds = Integer.parseInt(economyField.getText());
            int vipBeds = Integer.parseInt(vipField.getText());
            int icuBeds = Integer.parseInt(icuField.getText());
            
            // Validate economy beds (can increase/decrease, but must be >= available)
            if (economyBeds < 0 || economyBeds < currentHospital.getAvailableEconomyBeds()) {
                JOptionPane.showMessageDialog(mainFrame, "Economy beds must be >= 0 and >= available economy beds (" + currentHospital.getAvailableEconomyBeds() + ")", "Error", JOptionPane.ERROR_MESSAGE);
                return;
            }
            
            // Validate VIP beds (can increase/decrease, but must be >= available)
            if (vipBeds < 0 || vipBeds < currentHospital.getAvailableVipBeds()) {
                JOptionPane.showMessageDialog(mainFrame, "VIP beds must be >= 0 and >= available VIP beds (" + currentHospital.getAvailableVipBeds() + ")", "Error", JOptionPane.ERROR_MESSAGE);
                return;
            }
            
            // Validate ICU beds (can increase/decrease, but must be >= available)
            if (icuBeds < 0 || icuBeds < currentHospital.getAvailableIcuBeds()) {
                JOptionPane.showMessageDialog(mainFrame, "ICU beds must be >= 0 and >= available ICU beds (" + currentHospital.getAvailableIcuBeds() + ")", "Error", JOptionPane.ERROR_MESSAGE);
                return;
            }
            
            // Update all bed types
            boolean success = true;
            success &= currentHospital.setTotalEconomyBeds(economyBeds);
            success &= currentHospital.setTotalVipBeds(vipBeds);
            success &= currentHospital.setTotalIcuBeds(icuBeds);
            
            if (success) {
                int newTotalBeds = economyBeds + vipBeds + icuBeds;
                JOptionPane.showMessageDialog(mainFrame, "Total beds updated successfully!\nNew total: " + newTotalBeds + " beds\n(Economy: " + economyBeds + ", VIP: " + vipBeds + ", ICU: " + icuBeds + ")", "Success", JOptionPane.INFORMATION_MESSAGE);
            } else {
                JOptionPane.showMessageDialog(mainFrame, "Failed to update beds. Please check your values.", "Error", JOptionPane.ERROR_MESSAGE);
            }
        } catch (NumberFormatException ex) {
            JOptionPane.showMessageDialog(mainFrame, "Please enter valid numbers for all bed types.", "Error", JOptionPane.ERROR_MESSAGE);
        }
    }
}
...
private void showApproveRejectDialog() {
    ...
    private void showHospitalFullDetails() {
        // Show details of the currently logged-in hospital
        if (currentHospital == null) {
            JOptionPane.showMessageDialog(mainFrame, "No hospital is logged in.", "Error", JOptionPane.ERROR_MESSAGE);
            return;
        }
        
        showSingleHospitalDetails(currentHospital);
    }
